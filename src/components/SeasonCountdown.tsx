      ...
      <div className="flex flex-col items-center justify-between gap-5 md:flex-row">
        <div>
          ...
          <p className={`text-xs ${dark ? "text-white/60" : "text-slate-500"}`}>
            {new Date(season.start_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
            {" — "}
            {new Date(season.end_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
          </p>
        </div>
