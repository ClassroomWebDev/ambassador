...
import { isStaffRole, useCourses, useMyAttendance, useSessions } from "@/hooks/useBusiness";
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">100% of scheduled classes completed</p>
                </div>
                {existing ? (
                  <Badge variant={existing.status === "approved" ? "default" : "secondary"}>{existing.status}</Badge>
                ) : (
                  <Button size="sm" disabled={busy === c.id} onClick={() => void claim(c.id)}>
                    {busy === c.id ? <Loader2 className="size-3.5 animate-spin" /> : <BadgeCheck className="size-3.5" />}{" "}
                    Claim certificate
                  </Button>
                )}
              </article>
          ...
          mine.map((c) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">{c.serial_no ?? "Awaiting approval"}</p>
              </div>
              <div className="flex items-center gap-2">
